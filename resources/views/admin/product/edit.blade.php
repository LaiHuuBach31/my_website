...
@section('main')
<div class="container">
    <h1 class="text-center mb-5">Update Product</h1>
    <div class="row">
        <form action="{{route('product.update', $product->id)}}" method="post" enctype="multipart/form-data">
            ...
            <div class="col-lg-8">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="category_id" id="">
                        ...
                        <option value="{{$item->id}}" {{$product->category_id == $item->id ? 'seleted' : ''}}>{{$item->name}}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                    <small style="font-weight: 700; color: red;">{{$message}}</small>
                ...
                <!-- <div class="form-group">
                    ...
                    <input type="text" name="sale_price" value="{{$product->sale_price}}" class="form-control" placeholder="">
                </div> -->
                <div class="form-group">
            ...
            <div class="col-lg-4">
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    <img src="{{url('uploads')}}/{{$product->image}}" id="avatar" width="70px" alt="">
                    @error('upload')
                    <small style="font-weight: 700; color: red;">{{$message}}</small>
                    @enderror
                </div>
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    @foreach($images as $key => $item)
                        <img src="{{url('uploads')}}/{{$item->value}}" id="avatar_{{$key + 1}}" alt="" width="70px">
                    @endforeach
                    @error('uploads')
                    <small style="font-weight: 700; color: red;">{{$message}}</small>
                    @enderror
                    ...
                    <div style="display: flex; align-items: center; ">
                        ...
                        <div class="form-check" style="width: 33%;">
                            <label class="form-check-label">
                                <input type="checkbox" class="form-check-input" name="attr_id[]" value="{{$item->id}}" {{in_array($item->id, $attr_id) ? 'checked' : ''}}>
